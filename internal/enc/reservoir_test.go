		...
		for sri := range 3 {
			for _, nch := range []int{1, 2} {
				...
				for occ := 0; occ <= cap; occ++ {
					r := reservoir{occ: occ}
					lo, hi := r.spendBounds(area, cap, nch)
					if lo > hi {
...
func TestPlanFrameSplit(t *testing.T) {
	...
	lo, _ := rFull.spendBounds(area, cap, 2)
	if huff != 0 || spend != lo {
...
func TestPlanFrameDecoupling320k(t *testing.T) {
	...
	// 1440 - 4 - 17), huffCap = 2*4095/8 = 1023 < area, cap = 511. At full
	// occupancy with maximal demands: lo = 511+1419-511 = 1419 EXCEEDS the
	// Huffman capacity; the pre-decoupling design (hi folded the field
	// cap) had lo > hi here, a panic. Now: huffTarget = min(ceil(8190/8)=
	// 1024, min(1930, 1023)) = 1023, spend = max(1023, 1419) = 1419, and
	// the 396-byte delta is ancillary burn.
	...
	if huff != 1023 || spend != 1419 {
		t.Fatalf("(spend, huff) = (%d, %d), want (1419, 1023)", spend, huff)
	}
	if budgets[0]+budgets[1] != 1023*8 || budgets[0] > maxPart23Length || budgets[1] > maxPart23Length {
		t.Fatalf("budgets %v: sum %d, want %d with both within the field cap",
			budgets, budgets[0]+budgets[1], 1023*8)
	}
...
func TestReservoirSimulation(t *testing.T) {
	...
	for f := range 400 {
		...
		lo, hi := r.spendBounds(area, cap, 2)
